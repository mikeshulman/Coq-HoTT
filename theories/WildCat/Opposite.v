...
Global Instance is0coh1fun_op  A `{Is0Coh1Cat A} B `{Is0Coh1Cat B}
       (F : A -> B) `{!Is0Coh1Functor F}
  : Is0Coh1Functor (F : A ^op -> B ^op).
Proof.
...
Global Instance is1coh1fun_op A B `{Is0Coh1Cat A} `{Is0Coh21Cat B}
       (F : A -> B) `{!Is0Coh1Functor F, !Is1Coh1Functor F}
  : Is1Coh1Functor (F : A^op -> B^op).
Proof.
...
Definition transformation_op {A} {B} `{Is0Coh1Cat B}
           (F : A -> B) (G : A -> B) (alpha : F $=> G)
  : (@Transformation (A^op) (B^op) (@isgraph_1cat _ (is0coh1cat_op B))
